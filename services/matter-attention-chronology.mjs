...
export async function buildChronologyAttentionItems({ root, status } = {}) {
  const items = [];
  const caseTimelineJson = await readJsonFile(path.join(root, CASE_TIMELINE_JSON_RELATIVE));
  const markdownExists = await fileExists(path.join(root, CASE_TIMELINE_MARKDOWN_RELATIVE));
  const caseTimelineStage = stageBySlash(status, "/create_listofdates");
  ...
  if (caseTimelineJson.exists && !caseTimelineJson.valid) {
    addItem(items, {
      ...
      evidence: [evidence(CASE_TIMELINE_JSON_RELATIVE)],
    });
  }
  ...
  if (markdownExists && !caseTimelineJson.exists) {
    addItem(items, {
      ...
      evidence: [evidence(CASE_TIMELINE_MARKDOWN_RELATIVE), evidence(CASE_TIMELINE_JSON_RELATIVE)],
    });
  }
  if (!markdownExists && caseTimelineJson.exists) {
    addItem(items, {
      ...
      evidence: [evidence(CASE_TIMELINE_JSON_RELATIVE), evidence(CASE_TIMELINE_MARKDOWN_RELATIVE)],
    });
  }
