...
export function createRuntimeDbStorageService({
  ...
  async function enqueueProcessingJob({ matter, kind, idempotencyKey, metadata = {}, priority = 0 } = {}) {
    ...
    return withRuntimeDbClient(async (client) => {
      ...
      const result = await client.query([
        ...
        cleanUploadText(idempotencyKey, 240) || `runtime-job:${normalizedMatter.id}:${kind}:${randomUUID()}`,
        actor?.id || null,
        priority,
  ...
  async function listProcessingJobs({ matterName = "", kind = "", status = "", limit = 100 } = {}) {
    ...
    return withRuntimeDbClient(async (client) => {
      const result = await client.query([
        ...
        "limit $4::int",
      ].join("\n"), [cleanUploadText(matterName, 300), cleanUploadText(kind, 80), cleanUploadText(status, 80), Math.max(1, Math.min(Number(limit) || 100, 500))]);
  ...
  async function claimNextProcessingJob({ workerId = "runtime-worker", kinds = ["extract"], lockMs = 5 * 60 * 1000 } = {}) {
    ...
    return withRuntimeDbClient(async (client) => {
      await client.query([
        ...
        "set status = 'retrying', locked_by = null, locked_at = null, lock_expires_at = null, updated_at = now()",
  ...
  async function failProcessingJob(jobId, error, patch = {}) {
    ensureEnabled();
    const message = cleanUploadText(error?.message || error || "Processing failed", 500);
    const code = /^[a-z][a-z0-9_]*(?:\.[a-z0-9_]+)*$/.test(String(error?.code || "")) ? String(error.code) : cleanUploadText(patch.errorCode || "processing.failed", 120);
    return withRuntimeDbClient(async (client) => {
      const result = await client.query([
        "update processing_jobs",
...
function normalizeRuntimeProcessingJobRow(row = {}) {
  if (!row) return null;
  return {
    id: stringValue(row.id),
    matterId: stringValue(row.matter_id ?? row.matterId),
    ...
    idempotencyKey: stringValue(row.idempotency_key ?? row.idempotencyKey),
    attemptCount: positiveUploadInteger(row.attempt_count ?? row.attemptCount, 0),
    maxAttempts: positiveUploadInteger(row.max_attempts ?? row.maxAttempts, 0),
    progress: parseUploadJsonObject(row.progress_json ?? row.progressJson),
    createdAt: isoStringOrEmpty(row.created_at ?? row.createdAt),
    updatedAt: isoStringOrEmpty(row.updated_at ?? row.updatedAt),
