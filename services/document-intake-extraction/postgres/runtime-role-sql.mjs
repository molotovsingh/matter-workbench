...
export function buildDocumentIntakeExtractionRuntimeRoleSql({ roleName } = {}) {
  ...
  return [
    ...
    `grant select, insert, update on document_intake_extraction.worker_capacity_requests to ${role};`,
    `grant select, insert on document_intake_extraction.audit_events to ${role};`,
    ...
    `grant execute on function document_intake_extraction.claim_page_work(text, integer, jsonb) to ${role};`,
    `grant execute on function document_intake_extraction.claim_document_local_page_work(text, integer, integer, jsonb) to ${role};`,
...
export function buildDocumentIntakeExtractionReadRoleSql({ roleName } = {}) {
  ...
  return [
    ...
    `grant select on document_intake_extraction.worker_capacity_requests to ${role};`,
    `grant select on document_intake_extraction.audit_events to ${role};`,
    `grant execute on function document_intake_extraction.current_tenant_id() to ${role};`,
  ].join("\n");
}
