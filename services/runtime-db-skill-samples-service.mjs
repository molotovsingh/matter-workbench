...
function approveSampleSql({ tenantId, ideaId, sampleId, designBriefHash }) {
  const targetExistsSql = [
    "select 1",
    "from skill_samples target",
    "where target.tenant_id = current_app_tenant_id()",
    `  and target.idea_id = ${sqlString(ideaId)}`,
    `  and target.id = ${sqlString(sampleId)}`,
    `  and target.design_brief_hash = ${sqlString(designBriefHash)}`,
  ].join("\n");
  return [
    `select set_config('app.tenant_id', ${sqlString(tenantId)}, false);`,
    ...
    "    where ss.tenant_id = current_app_tenant_id()",
    `      and ss.idea_id = ${sqlString(ideaId)}`,
    `      and ss.id = ${sqlString(sampleId)}`,
    "      and ss.approved = true",
    "    limit 1",
    "  ) rows",
    "), (",
    "  select jsonb_build_object('errorCode', 'runtime_db.skill_sample.stale')",
    "  from skill_samples candidate",
    "  where candidate.tenant_id = current_app_tenant_id()",
    `    and candidate.idea_id = ${sqlString(ideaId)}`,
    `    and candidate.id = ${sqlString(sampleId)}`,
    `    and candidate.design_brief_hash <> ${sqlString(designBriefHash)}`,
    "  limit 1",
