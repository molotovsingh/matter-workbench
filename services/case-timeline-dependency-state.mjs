...
export { CASE_TIMELINE_DEPENDENCY_STATES };

export function classifyCaseTimelineDependencyState({ target, newestInput = {}, sourceIndex = null } = {}) {
  if (newestInput.inputKind !== "source_index") {
    return CASE_TIMELINE_DEPENDENCY_STATES.CHRONOLOGY_REGENERATION_NEEDED;
  }
  ...
  if (!sourceIndex || !Array.isArray(sourceIndex.sources) || !snapshot.length) {
    return CASE_TIMELINE_DEPENDENCY_STATES.CHRONOLOGY_REVIEW_NEEDED;
  }
  ...
  for (const previous of snapshot) {
    ...
    if (!current) return CASE_TIMELINE_DEPENDENCY_STATES.CHRONOLOGY_REGENERATION_NEEDED;
    compared += 1;
  ...
  return compared
    ? CASE_TIMELINE_DEPENDENCY_STATES.LABEL_REFRESH_NEEDED
    : CASE_TIMELINE_DEPENDENCY_STATES.CHRONOLOGY_REVIEW_NEEDED;
}
