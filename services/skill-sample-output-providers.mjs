...
export const SAMPLE_SYSTEM_PROMPT = legalWorkbenchSystemPrompt([
  ...
  "Use the proposed skill request, design brief, and available matter materials supplied by the app.",
  "The sample should look like the artifact the future skill would produce for the chosen test matter.",
  "Do not mention context packets, bounded context, evidence blocks, excerpts, truncation, omission, max limits, or other app/provider mechanics.",
  "If source coverage is incomplete, describe it in user-facing legal-work language such as 'available matter materials' or 'reviewed documents', not app-internal context language.",
...
export function sampleUserPayload({ idea, feedback, previousSample, matterContext }) {
  return {
    ...
    strict_boundaries: [
      ...
      "Do not mention context packets, bounded context, evidence blocks, excerpts, truncation, omission, max limits, or app/provider mechanics.",
      "When evidence is incomplete, explain it as a limitation of the reviewed matter materials rather than a limitation of the app context packet.",
