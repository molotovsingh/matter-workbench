...
export function buildWorkspaceSql({ tenantId, matter }) {
  return [
    ...
    "    coalesce(d.file_id, extraction_document.file_id, text_document.file_id, '') as file_id,",
    "    coalesce(d.original_name, extraction_document.original_name, text_document.original_name, '') as original_name,",
    "    coalesce(d.sha256, extraction_document.sha256, text_document.sha256, '') as document_sha,",
    "    coalesce(d.size_bytes, extraction_document.size_bytes, text_document.size_bytes, 0)::bigint as document_size_bytes,",
    ...
    "  left join extraction_records er on er.storage_object_id = so.id and er.tenant_id = so.tenant_id and er.matter_id = so.matter_id and er.superseded_at is null",
    "  left join documents extraction_document on extraction_document.id = er.document_id and extraction_document.tenant_id = er.tenant_id and extraction_document.matter_id = er.matter_id",
    "  left join document_text_blocks dtb on dtb.storage_object_id = so.id and dtb.tenant_id = so.tenant_id and dtb.matter_id = so.matter_id",
    "  left join documents text_document on text_document.id = dtb.document_id and text_document.tenant_id = dtb.tenant_id and text_document.matter_id = dtb.matter_id",
    ...
    `    and (d.id is null or d.status not in ${INACTIVE_SOURCE_DOCUMENT_STATUS_SQL})`,
    `    and (extraction_document.id is null or extraction_document.status not in ${INACTIVE_SOURCE_DOCUMENT_STATUS_SQL})`,
    `    and (text_document.id is null or text_document.status not in ${INACTIVE_SOURCE_DOCUMENT_STATUS_SQL})`,
...
export function buildPayloadSql({ tenantId, matter, relativePath }) {
  ...
  return [
    ...
    "  left join document_blobs db on db.storage_object_id = so.id and db.tenant_id = so.tenant_id and db.matter_id = so.matter_id",
    "  left join documents d on d.id = db.document_id and d.tenant_id = db.tenant_id and d.matter_id = db.matter_id",
    "  left join extraction_records er on er.storage_object_id = so.id and er.tenant_id = so.tenant_id and er.matter_id = so.matter_id and er.superseded_at is null",
    "  left join documents extraction_document on extraction_document.id = er.document_id and extraction_document.tenant_id = er.tenant_id and extraction_document.matter_id = er.matter_id",
    "  left join document_text_blocks dtb on dtb.storage_object_id = so.id and dtb.tenant_id = so.tenant_id and dtb.matter_id = so.matter_id",
