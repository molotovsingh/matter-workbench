...
test("copilot web research availability requires flag, key, and answer provider", () => {
  ...
  const ready = createCopilotWebResearchService({
    env: { COPILOT_WEB_RESEARCH_ENABLED: "1", EXA_API_KEY: "exa-test" },
    webResearchAnswerProvider: async () => ({ answer_status: "not_found" }),
  });
...
test("copilot web research normalizes fake answer provider output", async () => {
  const service = createCopilotWebResearchService({
    ...
    webResearchAnswerProvider: async ({ question, config }) => ({
      answer_status: "answered",
      answer_markdown: `Research answer from public sources\n\n${question}`,
      public_sources: [{ id: "WEB-0001", title: "IBC", url: "https://example.test/ibc" }],
      research: { query: "NCLT IBC", result_count: config.maxResults },
  ...
  assert.deepEqual(answer.public_sources, [{ id: "WEB-0001", title: "IBC", url: "https://example.test/ibc" }]);
  assert.deepEqual(answer.research, { provider: "exa", query: "NCLT IBC", result_count: 4 });
});
