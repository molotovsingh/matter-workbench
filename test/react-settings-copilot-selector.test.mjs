...
const modelPolicyPath = new URL("../shared/model-policy.mjs", import.meta.url);
...
test("React Settings exposes Matter Copilot as a task-scoped model selector", async () => {
  ...
  const modelPolicy = await readFile(modelPolicyPath, "utf8");

  assert.match(source, /Matter Copilot/);
  assert.match(source, /copilotModelPresets/);
  assert.doesNotMatch(models, /COPILOT_MODEL_PRESETS\s*[:=]/);
  assert.match(modelPolicy, /COPILOT_MODEL_PRESETS/);
  assert.match(modelPolicy, /label: "Low"/);
  assert.match(modelPolicy, /model: "gpt-4o-mini"/);
  assert.match(modelPolicy, /label: "Medium"/);
  assert.match(modelPolicy, /model: "gpt-5\.4-mini"/);
  assert.match(modelPolicy, /label: "High"/);
  assert.match(modelPolicy, /model: "gpt-5\.4"/);
  assert.match(modelPolicy, /label: "Highest"/);
  assert.match(modelPolicy, /model: "gpt-5\.5"/);
  assert.match(modelPolicy, /provider: AI_PROVIDERS\.OPENAI_DIRECT/);
  assert.doesNotMatch(modelPolicy, /google\/gemini/);
