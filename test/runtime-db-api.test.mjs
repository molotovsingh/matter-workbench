...
function proceduralPostureFinalDiagnosisFixture() {
  return {
    ...
    simple_case_view: "This looks like a pre-filing matter from the current DB record. The legal team should confirm whether any proceeding exists before drafting.",
    court_forum: {
    ...
    legal_routes: [{
      ...
      route_summary: "First confirm whether a proceeding exists and collect the full record.",
      when_to_use: "the current record does not show a proceeding number or order",
      why_this_route: "the next drafting path depends on whether the matter is still pre-filing",
      court_or_forum: "Civil forum to be confirmed",
      statutory_references: ["Verify governing statute, limitation, and civil procedure before filing"],
      what_to_confirm: ["Forum", "Current stage", "Whether a proceeding exists"],
      priority: "primary",
    }],
    recommended_route: {
      route_number: 1,
      ...
      reason: "The supplied DB record does not show a filed proceeding.",
      next_step: "Collect current case status and complete source papers.",
    },
    next_best_actions: ["Confirm forum", "Confirm whether proceedings exist", "Collect complete source papers"],
