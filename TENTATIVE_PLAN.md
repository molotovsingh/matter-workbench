...
4. **Explicit legal web research mode** — do not silently add web search to ordinary Copilot Q&A. The default Copilot should stay matter-record grounded and cite only matter sources (`FILE-NNNN pX.bY`, Library outputs, search results). Future web search should be a clearly labeled legal-research capability, such as `/research_law` or a `Matter / Legal Research` mode, with external-source citations, date-accessed metadata, jurisdiction/source-type labels, and a confidentiality guard that warns users not to send client facts externally unless they intentionally choose a fact-specific research query.
