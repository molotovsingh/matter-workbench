...
export async function runCli(argv = process.argv.slice(2), env = process.env) {
  ...
  if (command === "run-current") {
    const report = await runCurrentProviderCandidate({
      ...
      candidateId: required(options, "candidate-id"),
      codeRevision: String(options["code-revision"] || ""),
      primaryCacheCandidateId: String(options["primary-cache-candidate"] || ""),
      primaryModel: String(options["primary-model"] || ""),
      primaryBatchStrategy: String(options["primary-batch-strategy"] || "balanced"),
...
function usage() {
  return [
    ...
    "       [--code-revision GIT_SHA] [--primary-model MODEL] [--primary-cache-candidate ID]",
    "       [--primary-batch-strategy balanced|document_ranges]",
