...
| Create Case Timeline | `POST /api/case-timeline`; legacy `POST /api/create-listofdates`; `runCreateListOfDates`; runtime DB `createListOfDates` | Generated artifact mutation | Job ledger; `10_Library/Case Timeline.*`; provider run metadata | Good projection seed with currentness caveat | Source removal should normally mark `chronology_regeneration_needed` before any regeneration. |
| Refresh Case Timeline labels | `POST /api/case-timeline/refresh-labels`; legacy `POST /api/create-listofdates/refresh-labels`; `refreshListOfDatesSourceLabels`; runtime DB label refresh | Generated artifact mutation | Job ledger; Case Timeline artifact update | Good projection seed | Cheap label refresh, not legal chronology regeneration. |
| Write Matter Story | `POST /api/matter-story`; `matterStoryService.runDisputeStory`; runtime DB `persistTextArtifacts`/`persistMatterJson` | Generated artifact mutation | Job ledger; Matter Story artifact; custom skill run-like metadata | Good projection seed | Downstream of current Case Timeline; must become stale if source set changes. |
...
| Switch active matter / clear active matter | `/api/switch-matter`; `/api/active-matter/clear` | UI/session state | Active matter state | Not Matter Log custody | Selecting a matter is not a matter-record mutation. Archive/reopen is separate matter lifecycle state. |
