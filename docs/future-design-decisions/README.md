...
| [Copilot Web Research Mode](copilot-web-research-mode.md) | Planned feature / working product note | Medium-High | Firm lawyers need current-law options, sections, rules, or case-law context beyond the matter record/model cutoff | Add a simple `[Ask] [Research]` UX or `/research` command. Before coding, resolve the model-policy task boundary, validate public source IDs, treat web snippets as untrusted, expose a config flag, and route `/research` before the skill router. |
