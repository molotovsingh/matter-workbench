...
| [React-Only Cutover And Database Transition](react-only-cutover-database-transition.md) | Current local contract / transition prep | High | Any product-shell route, legacy frontend deletion, or database-first-slice work | Keep `/` React-only. Delete retired browser UX safely, but migrate useful helpers first. Use `db/migrations/001_control_plane.sql`, `db/migrations/002_tenant_rls.sql`, `db/migrations/003_tenant_reference_integrity.sql`, and `scripts/db-migrate.mjs` as the preparatory database baseline. |
