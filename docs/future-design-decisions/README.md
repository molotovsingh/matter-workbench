...
| [Intelligent Feedback Triage Router](private-beta-feedback-capture.md#intelligent-feedback-triage-router) | Next implementation slice | High for beta operations | Feedback volume grows beyond obvious hard failures, or bug/UX/feature reports become ambiguous | Build a hybrid router: deterministic hard-failure overrides first, model-assisted structured classifier second, conservative `investigate` fallback on low confidence or malformed output. Keep it server-side and operator-only. |
