...
| [React-Only Cutover And Database Transition](react-only-cutover-database-transition.md) | Current local contract / runtime DB transition | High | Any product-shell route, legacy frontend deletion, runtime DB work, or hosted DB-worker slice | Keep `/` React-only. Delete retired browser UX safely, but migrate useful helpers first. Use `db/migrations/001_control_plane.sql` through `db/migrations/015_storage_object_payloads.sql`, runtime DB role setup, runtime smoke, and write-smoke evidence as the accepted local/private database baseline. |
| [Navigation Shell Transition](navigation-shell-transition.md) | Implementation contract draft; prototype accepted for review, not production implementation permission | High for beta polish | Final-candidate prototype is approved for production implementation | Implement only a narrow shell/layout refactor: black rail with active matter + Matter Record, app nav below, All matters affordance, scrollable Matter Story. Preserve current Matter Assistant behavior as protected functionality. |
